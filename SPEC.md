...
*   **展示与过滤**:
    ...
    *   分数语义为"相似度"，数值越大越相似。
    *   **Snippet 截断**: 以匹配关键词为中心截取摘要（前后各 30 字符），确保用户直接看到匹配上下文。
    *   **关键词高亮**: 使用 N-gram 算法提取查询文本的关键词（优先 3-gram，过滤无意义组合），在摘要中高亮显示，帮助用户理解匹配原因。
    *   **相似度阈值截断**: 用户可设置 Minimum Similarity Threshold（默认 0.70），低于此分数的结果将被过滤，后端通过 LanceDB 原生 `distance_range()` 实现，宁缺毋滥。
    *   **分数颜色标签**: 每个结果卡片显示相似度分数（0.00-1.00），并根据阈值显示不同颜色：
        *   绿色 (>= 高分阈值，默认 0.85)：高度相关
        *   蓝色 (>= 中分阈值，默认 0.75)：中度相关
        *   黄色 (< 中分阈值)：边缘相关
    *   **分数提示**: 
        *   悬浮分数：显示相似度含义说明（高度相关/相关/可能相关）
        *   悬浮结果：显示查询文本与结果文本的详细对比，帮助用户理解推荐原因
