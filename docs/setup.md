...
2. **Backend project path**: **首要步骤**。填入您克隆仓库后的 `backend/` 文件夹绝对路径。
3. **环境自动对齐**: 
   - 路径填入后，插件会自动探测其环境。
   - 如果探测到 `.venv` 或 `uv.lock`，状态行将变绿并显示 `✅ 已自动关联`。
   - 如果未发现环境，状态行将变为橙色 `⚠️ 未发现虚拟环境`，并配套出现 **[一键初始化环境]** 按钮。点击即可全自动完成配置。
4. **启动与控制**:
   - **Auto-start server**: 开启此项，后端将随插件自动启动和销毁。
   - **手动启动服务**: 如果需要手动干预，可点击此按钮。系统将自动执行冲突探测，在 Windows 上甚至能一键清理霸占端口的僵尸进程。
