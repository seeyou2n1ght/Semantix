...
3. **自适应分片与主线程让渡**: 全量构建索引引入双阈值分片（≤25 篇且 ≤150k 字符），并在批次间通过 `requestIdleCallback` 帧让渡机制让出浏览器主线程，避免 UI 卡顿。

---

## 5. 进程生命周期与韧性治理 (Process Governance & Self-Healing)

1. **宿主退出自毁**: 后端内置看门狗监听 `SEMANTIX_PARENT_PID`。Windows 下通过 Win32 API `GetExitCodeProcess` 探测父进程退出状态码（退出码 ≠ 259 `STILL_ACTIVE` 即判定死亡），彻底消除句柄残留导致的虚假存活，在 10 秒内完成伴生清理并自杀。
2. **孤儿进程树清理**: 前端启动前读取 `.semantix.pid` 锁文件，通过平台指令树状强杀遗留孤儿进程，确保端口与文件锁即时释放。
3. **自愈状态机与熔断**:
   - 前端心跳丢失后自动进入自愈流程，采取 3s / 6s / 15s 指数退避重试拉起本地进程。
   - 连续失败 3 次触发熔断机制，终止无休止重试，保护系统资源并向用户输出诊断引导。
   - 用户主动停止（Stop）会压制自动重启；心跳连接成功或用户手动点击“重置并重启”后重置熔断计数器。
